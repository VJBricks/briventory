package orm.models;

import org.jooq.DSLContext;
import org.jooq.UpdatableRecord;

/**
 * The {@code PersistableModel3} should be used for models that needs five {@link org.jooq.Record} to be persisted into
 * the database. Each time a {@link org.jooq.Record} is requested, the previous one (if exists), it also passed, to
 * perform data refresh operations on both, the model and the new record.
 *
 * @param <R1> the first precise subtype of {@link UpdatableRecord}.
 * @param <R2> the second precise subtype of {@link UpdatableRecord}.
 * @param <R3> the third precise subtype of {@link UpdatableRecord}.
 * @param <R4> the fourth precise subtype of {@link UpdatableRecord}.
 * @param <R5> the fifth precise subtype of {@link UpdatableRecord}.
 */
public interface PersistableModel5<R1 extends UpdatableRecord<R1>,
                                      R2 extends UpdatableRecord<R2>,
                                      R3 extends UpdatableRecord<R3>,
                                      R4 extends UpdatableRecord<R4>,
                                      R5 extends UpdatableRecord<R5>> extends PersistableModel {

  /**
   * Returns the first {@link org.jooq.Record}.
   *
   * @param dslContext the {@link DSLContext}.
   *
   * @return the first {@link org.jooq.Record}.
   */
  R1 getUpdatableRecord1(DSLContext dslContext);

  /**
   * Returns the second {@link org.jooq.Record} and refresh the model using the first {@link org.jooq.Record}.
   *
   * @param dslContext the {@link DSLContext}.
   * @param r1 the first {@link org.jooq.Record}, that has been refreshed from the database with autogenerated fields.
   *
   * @return the first {@link org.jooq.Record}.
   */
  R2 getUpdatableRecord2(DSLContext dslContext, R1 r1);

  /**
   * Returns the third {@link org.jooq.Record} and refresh the model using the second {@link org.jooq.Record}.
   *
   * @param dslContext the {@link DSLContext}.
   * @param r2 the second {@link org.jooq.Record}, that has been refreshed from the database with autogenerated fields.
   *
   * @return the first {@link org.jooq.Record}.
   */
  R3 getUpdatableRecord3(DSLContext dslContext, R2 r2);

  /**
   * Returns the fourth {@link org.jooq.Record} and refresh the model using the third {@link org.jooq.Record}.
   *
   * @param dslContext the {@link DSLContext}.
   * @param r3 the third {@link org.jooq.Record}, that has been refreshed from the database with autogenerated fields.
   *
   * @return the first {@link org.jooq.Record}.
   */
  R4 getUpdatableRecord4(DSLContext dslContext, R3 r3);

  /**
   * Returns the fifth {@link org.jooq.Record} and refresh the model using the fourth {@link org.jooq.Record}.
   *
   * @param dslContext the {@link DSLContext}.
   * @param r4 the fourth {@link org.jooq.Record}, that has been refreshed from the database with autogenerated fields.
   *
   * @return the first {@link org.jooq.Record}.
   */
  R5 getUpdatableRecord5(DSLContext dslContext, R4 r4);

  /**
   * Refresh the model with the fifth {@link org.jooq.Record}.
   *
   * @param r5 the fifth {@link org.jooq.Record}, that has been refreshed from the database with autogenerated fields.
   */
  void lastRefresh(R5 r5);

}
